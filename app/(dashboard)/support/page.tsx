...
export default function SupportPage() {
  ...
  useEffect(() => {
    // Fetch the user's apps for the related app dropdown
    fetch('/api/profiles/me')
      .then((res) => res.ok ? res.json() : null)
      .then(() => {
        // We just need the apps the user owns — fetch via the apps API
        fetch('/api/apps')
          .then((res) => res.ok ? res.json() : [])
          .then((data: unknown) => {
            if (Array.isArray(data)) {
              setUserApps(data.map((a: { id: string; name: string }) => ({ id: a.id, name: a.name })));
            }
          })
          .catch(() => setUserApps([]));
      })
